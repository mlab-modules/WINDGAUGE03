...
log_file.write("log_index;system_timestamp;diff_press[Pa];mag_x;mag_y;mag_z;mag_hdg[deg];spd_from_dp[m/s];temp[degC];gps_timestamp;gps_hdg[deg];gps_sogk[m/s];p_altimet[Pa];temp_altimet[degC];temp_sht[degC];hum_sht[Percent]\n")
...
while True:
    ...
    try:
        ...
        (altimet_t1, altimet_p1) = altimet.get_tp()

        temperature, humidity = sht.get_TempHum()
        ...
        if serialPort is not None:
            ...
            msg = ("%d;%s;%0.2f;%0.2f;%0.2f;%0.2f;%0.2f;%0.2f;%0.3f;;;;%0.3f;%0.3f;%0.3f;%0.3f\n"% (log_index, ts, dp, mag_x, mag_y, mag_z, hdg_ma, spd_from_dp, temp, altimet_p1, altimet_t1,temperature, humidity))
            sys.stdout.write("%s; %s; Dp: %+4.2f [Pa]; T: %2.3f [degC]; " % (str(log_index).zfill(4), ts, dp, temp))
            sys.stdout.write("P: %+4.2f [Pa]; T: %2.3f [degC]; " % (altimet_p1, altimet_t1))
            sys.stdout.write("Temp:  %2.3f [degC]; Hum: %2.3f [Percent]; " % (temperature, humidity))
            sys.stdout.write("MAG_X: %+4.2f;MAG_Y: %+4.2f;MAG_Z: %+4.2f;MAG_HDG: %+4.2f; SPD_W_DP: %+4.2f [km/h]\n" % (mag_x, mag_y, mag_z, hdg_ma, spd_from_dp))
            sys.stdout.flush()
            log_file.write(msg)
