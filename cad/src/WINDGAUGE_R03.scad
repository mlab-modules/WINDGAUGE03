...
module pipes()
{
    ...
    cbl_x = 0;
    curvedPipe([[3, PCB_y + d - R03_wall_thickness/2 - 6.3, PCB_z + d + 0.1 ],
                [-mid_body_horizontal + 3, -mid_body_horizontal + 1 , PCB_z + d ],
                [-mid_body_horizontal - 3, 0                   , PCB_z - 5 ],
                [-mid_body_horizontal - 4, mid_body_horizontal   , slip_ring_z ],
                [-13                   , mid_body_horizontal + 0 , slip_ring_z       ],
               ],
                4,
                [mid_body_horizontal/1.2, d/2 + 0.1,
                 mid_body_horizontal - 5],
                d,
                0);
    // Cabling ramp
    translate([0, PCB_y, PCB_z])
        ...
        (
            ...
            points = [ // 0 = top left front
                     [-R03_PCB_width/2 + 1, -0.01, R03_PCB_height*0.1 + d],
                     // 1 = top right
                     [R03_PCB_width/2 - 1, -0.01, R03_PCB_height*0.1 + d],
                     // 2 = bottom right
                     [R03_PCB_width/2 - 1, -0.01, R03_PCB_height*0.1],
                     // 3 = bottom left front
                     [-R03_PCB_width/2 + 1, -0.01, R03_PCB_height*0.1],
                     // 4 = top left back
                     [-R03_PCB_width/2 + 1, d/2 + R03_wall_thickness/2, R03_PCB_height*0.1 + d],
                     // 5 = bottom left back
                     [-R03_PCB_width/2 + 1, d/2 + R03_wall_thickness/2, R03_PCB_height*0.1],
                   ],
            faces = [ [0, 1, 2, 3], // front
                      [1, 4, 5, 2], // back
                      [4, 0, 3, 5], // left
    ...
    curvedPipe([[vup_x, vup_y_in                                      , vup_z_in ],
                ...
                3,
//                [R03_air_tube_diameter/2 + 0.1 + h, R03_air_tube_diameter/2 + 0.1 + h],
//                R03_air_tube_diameter + h,
    ...
    sensor_y = vup_y_out - R03_PCB_elevation - R03_PCB_connector_overlay
               + R03_sensor_depth;
//    sealing_ring_extension(vup_x, sensor_y, vup_y_out, vup_z_out, h);
...
//                R03_air_tube_diameter + h,
                ...
                0);
    // Sealing ring extension
//    sealing_ring_extension(vlp_x, sensor_y, vlp_y_out, vlp_z_out, h);
...
module drop_shape(drop_length, draft)
{
    difference()
    {
        resize(newsize=[R03_wide_D, R03_wide_D, 0])
        ...
        // problems with bridge printing.
            union()
            {
                translate([-R03_PCB_width/2 - R03_global_clearance,
                           ...
                           -R03_PCB_height - R03_PCB_top_rim - R03_global_clearance])
                    cube([R03_PCB_width + 2*R03_global_clearance,
                          R03_PCB_depth + R03_PCB_connector_overlay + R03_PCB_elevation
                          + 1.5*R03_global_clearance,
                          R03_PCB_height + 1.5*R03_global_clearance]);
                translate([-R03_PCB_width/2 - R03_global_clearance, 0, -R03_PCB_height - R03_PCB_top_rim - R03_global_clearance])
                    polyhedron
                    (
                        points = [ // 0 = bottom right
                                   [R03_PCB_width + 2*R03_global_clearance, 0,
                                    -R03_PCB_height*0.1 - R03_global_clearance - 5],
                                   // 1 = bottom left
                                   [0, 0, -R03_PCB_height*0.1 - R03_global_clearance - 5],
                                   // 2 = top right front
                                   [R03_PCB_width + 2*R03_global_clearance,
                                    -R03_PCB_connector_overlay - 3*R03_PCB_elevation,
                                    2*R03_global_clearance],
                                   // 3 = top left front
                                   [0, -R03_PCB_connector_overlay - 3*R03_PCB_elevation, 2*R03_global_clearance],
                                   // 4 = top right back
                                   [R03_PCB_width + 2*R03_global_clearance, 0, 2*R03_global_clearance],
                                   // 5 = top left back
                                   [0, 0, 2*R03_global_clearance],
                                 ],
                        faces = [ [3, 5, 4, 2], // top
                                  [1, 3, 2, 0], // front
                                  [0, 4, 5, 1], // back
...
//module sealing_ring_extension(x, sensor_y, pipe_y, z, h)
module sealing_ring_extension(x, sensor_y, pipe_y, z)
{
        translate([x, sensor_y + 0.01, z])
        {
            rotate([90, 0, 0])
                cylinder (h = R03_sensor_depth + 0.02,
                          d = R03_sensor_diameter);
            translate([0, abs(sensor_y) - abs(pipe_y), 0])
            {

                rotate([90, 0, 0])
                    cylinder (h = abs(sensor_y) - abs(pipe_y) + 0.01,
//                              d1 = R03_air_tube_diameter + h,
//                              d2 = R03_sealing_ring_thickness + h);
...
module WINDGAUGE03A_R03(draft = true)
{
    difference()
    {
        union()
        {
            ...
            translate([0, -R03_wide_D/2 - R03_wall_thickness, R03_venturi_tube_height])
            {
                difference()
                {
                    ...
                    // TOP cutout
                    translate([-R03_wide_D, -R03_wide_D, -3*R03_wide_D])
                        cube([R03_wide_D*2, R03_wide_D, 4*R03_wide_D]);
                }
                    // PCB elevation
                    translate([-R03_PCB_width/2, 0.01, -R03_PCB_top_rim])
                        polyhedron
                        (
            ...
            difference()
            {
                ...
                // Bolt hole
                translate([-R03_fin_holder_width,
                           R03_fin_holder_depth - R03_fin_holder_height/2,
                           R03_fin_holder_height/2])
                    rotate([0, 90, 0])
                        cylinder(h = 2*R03_fin_holder_width, d = M3_bolt_diameter);
                // Nut holes
                mirror_copy([1, 0, 0])
                    translate([R03_fin_holder_width/2 - M3_nut_height,
                               ...
                               R03_fin_holder_height/2])
                        rotate([0, 90, 0])
                            cylinder(h = 2*M3_nut_height, d = M3_nut_diameter, $fn=6);
            }
        ...
        // Fin cut-out - TODO remove double cut-out nonsense just to avoid M3_bolts
        translate([-R03_fin_width/2, 0, -R03_fin_length/2 + R03_fin_holder_depth/2 + R03_global_clearance])
            rotate([0, 90, 0])
                WINDGAUGE01A_R05();
        translate([-R03_fin_width/2, -2*M3_bolt_diameter, -R03_fin_length/2 + R03_fin_holder_depth/2 + R03_global_clearance])
            rotate([0, 90, 0])
                WINDGAUGE01A_R05();
                ...
                translate([-7.61/2 - 2, -2, 7])
                {
                    ...
                    rotate([0, -90, 0])
                        translate([2, 2, -2])
                        {
                            hull()
                            {
                                cylinder(h=5, d1=5, d2=7);
                                cube([4.5,4,3], center=true);
                            }
                        }
                }
            }
        ...
        translate([0, R03_wide_D/2 + 5, slip_ring_z])
            rotate([-90, 0, 0])
                cylinder (h = R01_vyska_prekryti_statoru + R04_zavit_vyska + 0.02,
                          r = S01_prumer_vnitrni/2 + 2.5*S01_sila_materialu
                              + 2*R03_global_clearance);

        // Venturi wide in
        translate([0, 0, R03_venturi_tube_height - wide_body_length - 0.01])
            cylinder (h = wide_body_length + 0.02, d = R03_wide_D);
        ...
        // Venturi narrow
        translate([0, 0, wide_body_length + exhaust_length - 0.01])
            cylinder (h = R03_narrow_D + 0.02, d = R03_narrow_D);
       ...
       translate([-R03_PCB_width/2 - M3_nut_diameter, -R03_wide_D/2,
            ...
            rotate([270, 90, 0])
            {
                ...
                translate([0, 0, -R03_wide_D + M3_nut_height/2])
                    cylinder (h = R03_wide_D, d = M3_bolt_diameter);
            }
